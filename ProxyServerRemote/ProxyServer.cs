...
namespace ProxyServerRemote
{
    internal class ProxyServer
    {
        ...
        private async Task HandleClientAsync(Socket plainSocket)
        {
            ...
            try
            {
                this.Clients.Add(plainSocket.RemoteEndPoint.ToString(), new ProxyClient(plainSocket, plainSocket.RemoteEndPoint.ToString(), ip, this.Port, Program.RemoteIP, Program.RemotePort));
            }
            catch (Exception ex)
            {
